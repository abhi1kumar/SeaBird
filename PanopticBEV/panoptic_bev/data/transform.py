...
class BEVTransform:
    ...
    def _scale(self, img, bev_msk, front_msk, weights_msk, sem_msk= None):
        ...
        if sem_msk is not None:
            in_msk_w, in_msk_h = sem_msk[0].size[0], sem_msk[0].size[1]
            out_msk_w, out_msk_h = int(in_msk_w * self.scale), int(in_msk_h * self.scale)
            sem_msk = [m.resize((out_msk_w, out_msk_h), Image.BILINEAR) for m in sem_msk]

        return img, bev_msk, front_msk, weights_msk, sem_msk
    ...
    @staticmethod
    def _random_flip(img, bev_msk, front_msk, weights_msk, sem_msk= None, det_annotations= None, width= 768):
        if random.random() < 0.5:
            ...
            if sem_msk is not None:
                sem_msk = [m.transpose(Image.FLIP_TOP_BOTTOM) for m in sem_msk]
            if det_annotations is not None and det_annotations['pts'].shape[0] > 0:
                det_annotations['pts'][:, :, 0] = width - det_annotations['pts'][:, :, 0]
                yaw = det_annotations['yaw']
                det_annotations['yaw'] = np.sign(yaw) * (np.pi - np.abs(yaw))

            return img, bev_msk, front_msk, weights_msk, sem_msk, det_annotations
        else:
            return img, bev_msk, front_msk, weights_msk, sem_msk, det_annotations
    ...
    def __call__(self, img, bev_msk, front_msk=None, weights_msk=None, cat=None, iscrowd=None, calib=None, sem_msk= None, det_annotations= None):
        # Random flip
        if self.random_flip:
            img, bev_msk, front_msk, weights_msk, sem_msk, det_annotations = self._random_flip(img, bev_msk, front_msk, weights_msk, sem_msk, det_annotations, width= self.bev_crop[0])
        ...
        if self.scale:
            img, bev_msk, front_msk, weights_msk, sem_msk = self._scale(img, bev_msk, front_msk, weights_msk, sem_msk)
        ...
        if sem_msk is not None:
            sem_msk = [np.array(m, dtype=np.int32, copy=False) for m in sem_msk]
            sem_msk = np.stack(sem_msk, axis=0)
        ...
        if sem_msk is not None:
            sem_msk = torch.from_numpy(sem_msk.astype(np.float))
        if cat is not None:
        ...
        return dict(img=img, bev_msk=bev_msk, front_msk=front_msk, weights_msk=weights_msk, cat=cat, iscrowd=iscrowd,
                    bbx=bbx, calib=calib, sem_msk=sem_msk, det_annotations= det_annotations)
