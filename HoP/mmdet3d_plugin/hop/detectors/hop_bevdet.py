...
def clip_sigmoid_v1(x, eps=1e-4):
...
def get_bev_feat(sem_seg, img_feats, app_feat= "bev"):
    _, c, h, w  = img_feats[0].shape
    sem_seg_new = clip_sigmoid_v1(sem_seg['semantic_seg'].clone().float()) # b x 5 x Hseg x Wseg
    sem_seg_new = F.interpolate(sem_seg_new, (h, w), mode='bilinear')  # b x 5 x h x w
    # Convert to single channel segmentation channel
    sem_seg_single = torch.arange(1, sem_seg_new.shape[1] + 1, dtype= sem_seg_new.dtype, device= sem_seg_new.device)[None, :, None, None] * sem_seg_new  # b x 5 x h x w
    sem_seg_single = torch.max(sem_seg_single, dim= 1, keepdim= True)[0]  # b x 1 x h x w
    if app_feat == "bev":
        bev_feats_new = [torch.cat((img_feats[0][:, :(c - 1)], sem_seg_single), dim=1)]  # b x c x h x w
    else:
        raise NotImplementedError
    return bev_feats_new
...
class HoPBEVDet4D(BEVDet):
    ...
    def __init__(self,
        ...
        if seg_head is not None:
            self.seg_head = builder.build_seg_head(seg_head)
        else:
            self.seg_head = None
    ...
    def simple_test(self,
        ...
        if self.seabird:
            # Pass through BEV segmentation head
            sem_seg = self.seg_head[0](img_feats)                                         # b x 5 x Hseg x Wseg
            bev_feats_new = get_bev_feat(sem_seg, img_feats, app_feat= self.app_feat)
        else:
            bev_feats_new = img_feats

        bbox_list = [dict() for _ in range(len(img_metas))]
        bbox_pts = self.simple_test_pts(bev_feats_new, img_metas, rescale=rescale)
...
class HoPBEVDepth4D(HoPBEVDet4D):
    ...
    def forward_train(self,
        ...
        if self.seabird:
            ...
            targets = {}
            targets['semantic_map'] = semantic_map
            loss_seman = self.seg_head[0].loss(predictions= sem_seg, targets= targets)['loss_semantic_seg']
            losses.update(loss_seman=loss_seman)
        else:
            bev_feats_new = img_feats

        losses_pts = self.forward_pts_train(bev_feats_new, gt_bboxes_3d,
                                            gt_labels_3d, img_metas,
                                            gt_bboxes_ignore)
        losses.update(losses_pts)
        if self.with_hop:
