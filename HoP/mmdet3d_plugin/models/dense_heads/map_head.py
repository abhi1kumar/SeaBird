...
from .loss_utils import SegmentationLoss, BinarySegmentationLoss, DiceLoss
...
class MapHead(BaseTaskHead):
    def __init__(self, task_dict, in_channels, inter_channels=None,
        ...
        super(MapHead, self).__init__(
            task_dict, in_channels, inter_channels, init_cfg, norm_cfg, identity= identity)
        ...
        if self.loss_type == 'binary_cls':
            self.semantic_seg_criterion = BinarySegmentationLoss(
                pos_weight=pos_weight)
        elif self.loss_type == 'dice':
            self.semantic_seg_criterion = DiceLoss()
        else:
    ...
    def get_semantic_indices(self, predictions, targets=None):
        if self.loss_type == 'binary_cls':
            ...
            pred_semantic_indices = pred_semantic_indices.long()
             ...
             pred_semantic_scores = clip_sigmoid(predictions['semantic_seg'].clone().float())
             ...
             pred_semantic_indices = torch.max(pred_semantic_logits, dim=1)[0]
        else:
    ...
    def forward(self, x, targets=None):
        x = x[0]
        return {task_key:  F.interpolate(task_head(x), self.out_size, mode= 'bilinear') for task_key, task_head in self.task_heads.items()}
    ...
    def loss(self, predictions, targets):
        ...
        if self.loss_type == 'binary_cls':
            ...
            loss_dict['loss_semantic_seg'] = self.loss_weight * self.semantic_seg_criterion(
                clip_sigmoid(predictions['semantic_seg'].float()),
                targets['semantic_map'].float(),
            )
        elif self.loss_type == 'dice':
            # Convert to occupancy style stuff
            assert predictions['semantic_seg'].shape == targets['semantic_map'].shape
            loss_dict['loss_semantic_seg'] = self.loss_weight * self.semantic_seg_criterion(
                clip_sigmoid(predictions['semantic_seg'].float()),
                targets['semantic_map'].float(),
